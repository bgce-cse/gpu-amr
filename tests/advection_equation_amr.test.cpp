...
struct AdvectionTestConfig
{
    ...
    using geometry_t =
        amr::ndt::solver::physics_system<patch_index_t, patch_layout_t, domain_lengths>;
};

using AdvectionTestTypes =
    ::testing::Types<AdvectionTestConfig<2, 10>, AdvectionTestConfig<2, 50>>;
...
TYPED_TEST(AdvectionSolverTest, PulseCenterMovement)
{
    ...
    // Setup Solver
    amr_solver<
        typename Config::tree_t,
        typename Config::geometry_t,
        typename Config::equation_t,
        Config::DIM>
        solver(2000, 1.0, 0.4);
    ...
    auto ic = [&](auto const& coords) -> amr::containers::static_vector<double, 1>
    {
        ...
        for (int d = 0; d < Config::DIM; ++d)
            r2 += std::pow(coords[d] - start_pos[d], 2);
    ...
    {
        const auto  slot     = tree.get_active_slot_at(p_idx);
        const auto  patch_id = tree.get_node_index_at(slot);
        auto const& patch    = tree.template get_patch<amr::cell::Scalar>(slot);

        for (std::size_t l_idx = 0; l_idx < Config::patch_layout_t::flat_size(); ++l_idx)
        {
            if (amr::ndt::utils::patches::is_halo_cell<typename Config::patch_layout_t>(
                    l_idx
                ))
                continue;
            ...
            if (patch[l_idx] > max_val)
            {
                ...
                for (int d = 0; d < Config::DIM; ++d)
                    actual_pos[d] = coord[d] + 0.5 * size[d];
            }
        }
    }
    ...
    for (int d = 0; d < Config::DIM; ++d)
    {
        std::cout << "Expected pos (" << d << "): " << expected_pos[d] << ", Actual pos ("
                  << d << "): " << actual_pos[d] << std::endl;
...
TYPED_TEST(AdvectionSolverTest, PulseCenterMovementAMR)
{
    ...
    auto amrCriterion = [&](const Config::patch_index_t& idx)
    {
        ...
        if (max_val > 0.1 && idx.level() < 5)
            return Config::tree_t::refine_status_t::Refine;
        if (max_val < 0.05 && idx.level() > 1)
            return Config::tree_t::refine_status_t::Coarsen;
    ...
    for (int d = 0; d < Config::DIM; ++d)
    {
        std::cout << "Expected pos (" << d << "): " << expected_pos[d]
                  << ", Actual pos with AMR (" << d << "): " << actual_pos[d]
                  << std::endl;
        EXPECT_NEAR(actual_pos[d], expected_pos[d], cell_size[d])
            << "Pulse center mismatch in dimension " << d;
    }
