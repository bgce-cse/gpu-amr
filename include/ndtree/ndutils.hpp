...
public:
    ...
    static constexpr auto all_child_linear_idxs_in_dir(size_type direction
    ) -> std::array<size_type, utility::cx_functions::pow(s_1d_fanout, s_dimension - 1)>
    {
        size_t              dim_direction = direction / 2;
        bool                positive      = (direction % 2) == 1;
        constexpr size_type num_boundary_children =
            utility::cx_functions::pow(s_1d_fanout, s_dimension - 1);
        std::array<size_type, num_boundary_children> indices{};
        std::array<size_type, s_dimension>           coords{};
        coords[dim_direction] = positive ? (s_1d_fanout - 1) : 0;

        size_type idx = 0;
        for (size_type dim = 0; dim < s_dimension; dim++)
        {
            if (dim == dim_direction) continue;

            for (size_type i = 0; i < s_1d_fanout; i++)
            {
                coords[dim]  = i;
                indices[idx] = compute_child_linear_idx(coords);
                idx++;
            }
        }
    ...
    } // End of compute_child_neighbors
