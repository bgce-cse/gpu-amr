...
template <
    ...
    containers::concepts::Container auto Domain_Sizes>
...
private:
    ...
    static constexpr std::uint32_t s_max_morton_coord = 1u << s_max_depth;
...
public:
    ...
    [[nodiscard]]
    static constexpr auto patch_sizes(patch_index_t const& morton_id) noexcept
        -> physics_coord_arr_t
    {
        auto const& [coords, level] = patch_index_t::decode(morton_id.id());
        ...
        for (size_type i = 0; i != n_dimension; ++i)
        {
            result[i] = s_physics_lengths[i] *
                        static_cast<physics_coord_t>(patch_size_morton) /
    ...
    [[nodiscard]]
    static constexpr auto cell_sizes(patch_index_t const& morton_id) noexcept
        -> physics_coord_arr_t
    {
        const auto          patch_size = patch_sizes(morton_id);
        physics_coord_arr_t result{};
        for (size_type i = 0; i < n_dimension; ++i)
        {
    ...
    [[nodiscard]]
    static constexpr auto patch_coord(patch_index_t const& morton_id) noexcept
        -> physics_coord_arr_t
    {
        auto const& [coords, level] = patch_index_t::decode(morton_id.id());
    ...
    [[nodiscard]]
    static constexpr auto cell_coord(
        patch_index_t const&                              morton_id,
        typename patch_layout_t::padded_layout_t::index_t linear_idx
    ) noexcept -> physics_coord_arr_t
    {
    ...
    [[nodiscard]]
    static constexpr auto domain_lengths() noexcept -> physics_coord_arr_t const&
    {
        return s_physics_lengths;
    }

    [[nodiscard]]
    static constexpr auto domain_length(size_type dim) noexcept -> physics_coord_t
    {
