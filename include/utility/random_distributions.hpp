...
struct distribution<T, DistributionCategory::Normal>
{
    using distribution_t =
        std::normal_distribution<std::common_type_t<T, float>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
{
    using distribution_t =
        std::exponential_distribution<std::common_type_t<T, float>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
{
    using distribution_t =
        std::gamma_distribution<std::common_type_t<T, float>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
{
    using distribution_t =
        std::geometric_distribution<std::common_type_t<T, int>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
{
    using distribution_t =
        std::binomial_distribution<std::common_type_t<T, int>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
{
    using distribution_t =
        std::student_t_distribution<std::common_type_t<T, float>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
{
    using distribution_t =
        std::discrete_distribution<std::common_type_t<T, int>>;
    using result_type = typename distribution_t::result_type;
    using param_type = typename distribution_t::param_type;
};
...
template <
    utility::concepts::Arithmetic T, DistributionCategory Distribution,
    ...
    requires(
        ...
        (std::is_integral_v<T> and
         (std::is_integral_v<
              typename impl::distribution<T, Distribution>::result_type> or
...
class random_distribution
{
  ...
  public:
    random_distribution(
        param_type params, unsigned int seed = std::random_device{}()
    ) noexcept
        : random_engine_(seed)
    ...
    inline auto operator()() noexcept -> result_type
    {
                           ...
                           std::is_floating_point_v<impl_result_type>)
        {
            ...
            else if constexpr (discretizaton_policy ==
                               DiscretizationPolicy::Ceil)
            {
                return static_cast<result_type>(std::ceil(value));
            }
            else if constexpr (discretizaton_policy ==
                               DiscretizationPolicy::Trunc)
            {
                return static_cast<result_type>(std::trunc(value));
            }
            else if constexpr (discretizaton_policy ==
                               DiscretizationPolicy::Round)
            {
