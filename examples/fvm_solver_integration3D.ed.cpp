...
int main() {
    ...
    auto acousticPulseIC_3D = [](double x, double y, double z) -> amr::containers::static_vector<double, 5> {
    ...
    while (t < tmax) {
        ...
        //     auto& rhow_patch = solver.get_tree().template get_patch<amr::cell::Rhow>(patch_idx);
        //     auto& e_patch = solver.get_tree().template get_patch<amr::cell::E3D>(patch_idx);
