...
int main() {
    ...
    uint64_t morton_code = morton3d::encode3D(x, y, z, level);
    std::cout << "Encoded 3D Morton code: " << morton_code << std::endl;

    // Decode the Morton code back into coordinates
    auto decoded_coords = morton3d::decode3D(morton_code);
    ...
    level = static_cast<uint8_t>(temp_level);
    uint64_t morton_code_child = morton3d::encode3D(x, y, z, level);
    uint64_t morton_parent = morton3d::getParent3D(morton_code_child);
    auto decoded_parent_coords = morton3d::decode3D(morton_parent);
