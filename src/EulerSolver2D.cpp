// Compile with: g++ -O2 -o test_quadrants test_quadrants.cpp EulerSolver2D.cpp
...
EulerSolver2D::EulerSolver2D(int nx_, int ny_, double Lx, double Ly, double gamma_, double cfl_)
...
void EulerSolver2D::conservativeToPrimitive(const std::vector<double>& cons, std::vector<double>& prim) const {
    double rho = cons[0];
    double u = cons[1] / rho;
    double v = cons[2] / rho;
    double E = cons[3];
...
void EulerSolver2D::primitiveToConservative(const std::vector<double>& prim, std::vector<double>& cons) const {
    double rho = prim[0];
    double u = prim[1];
    double v = prim[2];
    double p = prim[3];
...
// Compute flux vector F (x-direction) or G (y-direction) from conservative variables
void EulerSolver2D::computeFlux(const std::vector<double>& cons, std::vector<double>& flux, int direction) const {
...
double EulerSolver2D::computeSoundSpeed(const std::vector<double>& cons) const {
    std::vector<double> prim(4);
    conservativeToPrimitive(cons, prim);
    double rho = prim[0];
    double p = prim[3];
...
void EulerSolver2D::rusanovFlux(const std::vector<double>& UL, const std::vector<double>& UR, 
...
double EulerSolver2D::computeTimeStep() const {
    double dt_min = 1e10;
...
void EulerSolver2D::applyBoundaryConditions() {
...
void EulerSolver2D::initializeCustom(std::function<std::vector<double>(double, double)> init_func) {
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            double x = i * dx;
            double y = j * dy;
...
void EulerSolver2D::timeStep(double dt) {
...
void EulerSolver2D::writeToVTK(const std::string& filename) const {
...
void EulerSolver2D::solve(double tmax, int output_interval, const std::string& output_prefix) {
    double t = 0.0;
    int step = 0;
    int file_counter = 0;
